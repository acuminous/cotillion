...
function createSystem(definition, options) {
  ...
  function runStart() {
    const deadline = createDeadline('start', timeouts.start, waits, stopOnExpiry);
    const walk = startSystem(deadline).finally(deadline.clear);
    return { walk, deadline };
  }

  function stopOnExpiry() {
  ...
  function start() {
    ...
    starting ??= runStart();
    lastStart = starting.walk.catch(stopAfterFailure(starting.deadline));
    return lastStart;
  }
...
async function startComponent(system, entry, states, waits, deadline, started) {
  ...
  const result = await waits.waitFor('start', entry, signal, () => invokeStart(system, entry, started, signal));
