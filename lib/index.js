...
const { bindExitListeners } = require('./exit-listeners');
...
function createSystem(definition, options) {
  ...
  const system = Object.assign(new EventEmitter(), { name: options?.name });
  const timeouts = timeoutsOf(options?.timeouts);
  ...
  async function startSystem(deadline) {
    ...
    announce(system, SystemEvent.StartInitiated);
    const started = await startComponents(system, definition, states, waits, deadline)
      .finally(clearStartDeadline)
      .catch(announceSystemFailure(system, SystemEvent.StartFailed, began));
    Object.assign(components, started);
    if (deadline.interruption) return components;
    starting = null;
    announce(system, SystemEvent.StartSucceeded, { duration: elapsedSince(began) });
    return components;
  }

  async function stopSystem(deadline) {
    const began = performance.now();
    announce(system, SystemEvent.StopInitiated);
    await interruptStart();
    await stopComponents(system, definition, states, waits, deadline).catch(
      announceSystemFailure(system, SystemEvent.StopFailed, began),
    );
    resetSystem();
    announce(system, SystemEvent.StopSucceeded, { duration: elapsedSince(began) });
  }
  ...
  function stopOn(...signals) {
    validateEvents(signals);
    for (const signal of signals) process.on(signal, stopQuietly);
    return () => {
      for (const signal of signals) process.off(signal, stopQuietly);
    };
  }

  function exitOn(...signals) {
    const unbindStop = stopOn(...signals);
    const unbindExit = bindExitListeners(system);
    return () => {
      unbindStop();
      unbindExit();
    };
...
const startOutcomes = {
  [Outcome.Succeeded]: (system, entry, states, { value, duration }) => {
    system.emit(ComponentEvent.StartSucceeded, { name: entry.name, duration });
    states.set(entry, SkipReason.Started);
    return value;
  },
  [Outcome.Failed]: (system, entry, states, { error, duration }) => {
    system.emit(ComponentEvent.StartFailed, { name: entry.name, error, duration });
    states.set(entry, SkipReason.Failure);
    throw error;
  },
  [Outcome.Aborted]: (system, entry, states, { error, duration }, deadline) => {
    system.emit(ComponentEvent.StartAborted, { name: entry.name, reason: deadline.interruption.reason, duration });
...
const stopOutcomes = {
  [Outcome.Succeeded]: (system, entry, states, { duration }) => {
    system.emit(ComponentEvent.StopSucceeded, { name: entry.name, duration });
    states.set(entry, SkipReason.Stopped);
  },
  [Outcome.Failed]: (system, entry, states, { error, duration }) => {
    system.emit(ComponentEvent.StopFailed, { name: entry.name, error, duration });
    throw error;
  },
};

function announce(system, event, payload) {
  system.emit(event, { name: system.name, ...payload });
}

function announceSystemFailure(system, event, began) {
  return (error) => {
    announce(system, event, { error, duration: elapsedSince(began) });
    throw error;
  };
}

function elapsedSince(began) {
