...
function createSystem(definition) {
  ...
  async function runStart(deadline) {
    stopping = null;
    unstarted = [];
    system.emit(SystemEvent.StartInitiated);
    const started = await startComponents(system, definition, standing, unstarted, deadline).catch(
  ...
  async function runStop(deadline) {
  ...
  function beginStart(deadline) {
    starting ??= runStart(deadline);
    return starting;
  }

  function beginStop(deadline) {
    stopping ??= runStop(deadline).catch(allowRetry);
    return stopping;
  }
