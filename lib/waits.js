...
function createWait(lifecycle, entry, deadlineSignal) {
  ...
  const timeout = timeoutsOf(entry.timeouts)[lifecycle];
  const expiry = schedule(timeout, expire);
  const began = performance.now();

  function begin(invoke) {
    invoked(() => invoke(signal)).then(
      (value) => settle({ outcome: Outcome.Succeeded, value }),
      (error) => settle({ outcome: rejectionOutcome(signal), error }),
    );
  }

  function fail(error) {
    settle({ outcome: Outcome.Failed, error });
  }

  function settle(result) {
    resolve({ ...result, duration: performance.now() - began });
  }
